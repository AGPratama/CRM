...
.controller('CalendarSiteVisitCtrl', function($scope, schedule, lokasi, $ionicLoading, $ionicModal, $ionicPlatform, $state, $cordovaGeolocation, radius, $ionicScrollDelegate) {
        ...
        $scope.confirmBerangkat = function(){
                console.log(sc);
        };
...
//                    var utcends = new Date(ends.getUTCFullYear(), ends.getUTCMonth(), ends.getUTCDate(),  ends.getUTCHours(), ends.getUTCMinutes(), ends.getUTCSeconds());
        ...
        $scope.onEventSelected = function (event) {
            ...
            $scope.status_app_name = event.status_app_name;
            $scope.memo = event.memo;
            if(event.status_app_id==12)
            {
                $scope.berangkat = true;
                $scope.checkin = false;
            }
            else
            if(event.status_app_id==9)
            {
                $scope.berangkat = false;
                $scope.checkin = true;
            }
            ...
            $scope.sc = sc["schedules"][event.schedule_employee_id];  
